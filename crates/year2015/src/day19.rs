...
impl Day19 {
    ...
    pub fn part1(&self) -> usize {
        // Total molecules generated by the rules, before deduplication
        let mut counts = [0; Atom::ALL.len()];
        ...
        let total = self
            .rules
            .iter()
            .filter_map(|(from, _)| from.map(|from| counts[from as usize]))
            .sum();

        let mut set = FastSet::with_capacity(total);
